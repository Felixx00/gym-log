...
│   │   ├── program.tsx          # Read-only program view (completed programs)
│   │   └── day.tsx              # Read-only day view (logged workout data)
│   └── settings/
│       ├── _layout.tsx          # Stack navigator
...
- **Library**: `library/index.tsx` displays two sections: a featured "Active Routine" card at the top (if one is set) and an "All Routines" list below. Three-dots menu provides Set as Active / Remove Active, Edit, and Delete. Only one program can be active at a time. Completed programs show a green "Done" badge; tapping a completed card opens a read-only program view (`library/program.tsx`) where completed days can be tapped to view logged data (`library/day.tsx`). The active card shows "COMPLETED" header with green border when the program is fully done. A circular "+" button navigates to `library/create.tsx` which collects name/weeks/days (name required, weeks via slider 4–20 default 12, days/week via pill selector 3–12 default 5) → pushes to `weeks.tsx` → user builds program → Save validates all weeks (day names required + unique per week, exercises required per day, exercise name + rep range required) → writes full tree to SQLite in one transaction → returns to Library list.
